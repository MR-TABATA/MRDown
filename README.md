...
- `⌘E` starts editing: **the preview becomes the editor in place**, and `⌘E` turns it back. The swap keeps the paragraph you were reading, so you read and write at the window's full width. Settings (`⌘,`) ▸ Appearance ▸ "Editing layout" switches to **side by side** instead (editor + live preview, drag the divider to resize). Save with `⌘S`, or "Save As…" (`⌘⇧S`) to write a copy elsewhere.
