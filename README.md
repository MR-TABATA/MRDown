...
- **The editor reads as the document while you write it**: headings are big, bold is bold, and markup like `#` or ``` is hidden. Checklists become real checkboxes you can click, and images render where they sit. **The line the caret is on always shows its raw markup**, so the Markdown is one keystroke away whenever you want it. The buffer stays plain text throughout, so saving never rewrites a byte of your file (tables are still shown as markup).
