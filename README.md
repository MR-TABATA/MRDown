...
- 見出しのアウトライン（目次）を本文の右に独立した列で表示：クリックでその見出しへスクロールし、読んでいる位置の見出しが自動でハイライトされます。`⌘2` で開閉、編集中も内容にライブ追従。設定（`⌘,`）で左サイドバー内に戻すこともできます（そのときは折りたたみ可）。位置も開閉状態も記憶されます。 / Outline (table of contents) in a column of its own, to the right of the document: click a heading to jump to it, and the heading you're reading is highlighted as you scroll. Toggle it with `⌘2`; it follows the document live while you edit. Settings (`⌘,`) can move it back into the left sidebar instead (where it stays collapsible). Both the position and the open/closed state are remembered.
