...
[![MRDown showing the three-column view after a file was rewritten while the user was editing it: last save, what is on disk, and the user's own unsaved edits, with only the conflicting lines marked](docs/media/demo-en.jpg)](docs/media/demo-en.mp4)

*An agent rewrote the file while there were unsaved edits in the window. MRDown neither reloads over you nor overwrites what arrived — it shows **last save / on disk / my edits** side by side and lets you choose. [Watch the 20-second demo.](docs/media/demo-en.mp4)*

<p align="center">
  <img src="docs/media/shot-timeline-en.png" width="49%" alt="The History panel comparing a version from 14 days ago with the current text, with HEAD (Git) in the same list">
  <img src="docs/media/shot-compare-en.png" width="49%" alt="Two separate files compared in the same view, word-level changes marked">
</p>
...
It began with the `.md` files Claude Code generates. Opening a heavy editor and right-clicking to
preview was a chore — so I asked Claude Code for help and had a **viewer-only 0.1 in two hours**.
At first it was a split — **write in an editor, read in MRDown**. That split is gone: **I no longer
open VSCode for `.md` at all**, I read and write here.

What grew out of that is the part no other Markdown app covers. When an agent rewrites a file
under you, an ordinary editor either reloads over your unsaved edits or overwrites what the agent
wrote — and either way, one of the two versions is gone. MRDown keeps both: every save, and
**every rewrite from outside the app**, quietly becomes a timestamped version, and the History
panel puts **any two of them side by side**. In a Git repository `HEAD` joins the same list, so the
diff against what's committed is right there — but Git is not required, and versions live in the
app's data directory, so they never clutter your files or your repos.
...
### The diff — the reason this exists
...
- **Drop a Word document (`.docx`) too** — it opens as Markdown. The conversion happens on your machine and nothing is uploaded. What opens is an *untitled* document, so saving writes a new `.md` and never touches the original `.docx`. Images are not carried over (text, headings, lists and tables are). Feed two `.docx` files to Compare and you get the diff between two versions of a document.
...
- **Turn a rough list into a numbered WBS**, and edit tables in place. One press numbers a task list `1`, `1.1`, `1.2` **with its nesting intact** — press again after deleting an item and the gaps close, because existing ids are stripped before renumbering. Table columns are added, deleted and aligned from the ▾ beside the table button (or `/`) — alignment cycles left → centre → right → none — and a separate action pads every cell so the source's pipes line up, without changing how it renders. `/wbs` drops in a skeleton, and it is the only `/` part that opens a form — depth, ids, start date, and the companion table and gantt, all reachable with Enter alone.
- **Nine small transforms for the selection**: letter case (cycles camel → snake → kebab → CONSTANT → Title), sorting (by text, by number — "10, 2, 33" becomes 2, 10, 33 — or by length), join and split, wrap at 80 columns, dedent, and decoding `&amp;` / `%E6%97%A5`. With nothing selected they act on the caret's line. Off by default: pick the ones you want in Settings ▸ Toolbar.
...
- Copy any folder's structure as plain text — a button on the folder header, or on each subfolder row on hover — every file, not just Markdown (the same scope as the `tree` command), wrapped in a code fence so pasting it into a document keeps the layout. Past 1,000 entries it asks first, in case something like `node_modules` got swept in.
